...
void save_current_data_size(int data_size){
...
unsigned int get_file_free_space() {
	ifstream current_file(full_path, ifstream::binary);
	current_file.seekg(0, current_file.end);
	unsigned int current_file_size = static_cast<unsigned int>(current_file.tellg());
...
void store(int id, void* data, int array_length) {
	...
	save_current_data_size(current_data_size);
	FILE * data_file = NULL;
	...
	while (current_data_size != 0) {
		...
		if (current_data_size > free_space) {
			...
			long int start_position = ftell(data_file);
			fwrite(&data, 1, current_data_size, data_file);
			long int end_position = ftell(data_file);
			save_file_name_and_reading_position(id, current_file, start_position, end_position);
